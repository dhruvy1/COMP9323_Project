...
{% if questions %}
  {% for question in questions %}
    <div class="row-fluid">
      <div class="col-xs-1 ans"><small><div class="ques">{{ question.num_answers }}</div>Answers</small></div>
      <div class="col-xs-1 ans"><small><div class="ques">{{ question.hit_count.hits }}</div>Views</small></div>
      <p>
        {% if question.reward %}{% endif %}
        <h4><a href="{% url 'qa_detail' question.id %}">{{ question.title }}</a></h4>
        <br/>
        {% for tag in question.tags.all %}
            <span class="label label-warning"><a href="{% url 'qa_tag' tag.slug %}">{{ tag.slug }}</span></a>
