...
def getFacebookEvents(page_name, result_limit):
    ...
    for fb_event in page_events["data"]:
        my_event = Event()  # default constructed event object
        ...
        # time
        my_event.start_date = getDateFromTime(fb_event["start_time"])
        my_event.end_date = getDateFromTime(fb_event["end_time"])
        my_event.start_time = filterTime(fb_event["start_time"])
        my_event.end_time = filterTime(fb_event["end_time"])

        # place - can be optional
        if "place" in fb_event:
            fb_event_place = fb_event["place"]
